<?php
	...
	$link_laps = mysqli_connect("localhost", "root", "", "laps");
	 ...
	 if (isset($_POST["num_parcelas"])) {$num_parcelas = $_POST['num_parcelas'];}
	...
	if($controle){
		while( $data=mysqli_fetch_assoc($result) ){ 
			  ...
			  if($num_parcelas == 1){
						  $insert = "INSERT INTO compras(id_compra,id_cartao,valor,quantidade,parcelas,categoria) values(".$data2['max'].",".$_POST["num_cartao"].",".str_replace(",",".",$data["valor"]).",".$data['addcar'].",'".$num_parcelas."','".$data['categoria']."');";
						  echo $insert."</br>";
						  ...
						  $compras_ok = 1;
									...
									for($i =1; $i <= $num_parcelas; $i++){
						                ...
						                $data2=mysqli_fetch_assoc($result2);
										 ...
										 $insert = "INSERT INTO compras(id_compra,id_cartao,valor,quantidade,parcelas,categoria) values(".$data2['max'].",".$_POST["num_cartao"].",".str_replace(",",".",$data["valor"]).",".$data['addcar'].",'".$i."/".$num_parcelas."','".$data['categoria']."');";
										  echo $insert."</br>";
										  mysqli_query($link_laps, $insert);	
										  //$insert = '';
										  $compras_ok = 1;
