...
if(!empty($_GET['recusa']))
{
	...
	$sql = "UPDATE Cliente  set cadastro = 0 where id_cliente = ".$user;
	if (mysqli_query($link, $sql)) { //echo "inserido";
	}
	$mail = $_GET['email'];
	include("solicitacao_recusa.php");
	$volta = $_SERVER['HTTP_REFERER'];
	echo "<script>window.location='$volta';alert('$nome, Cliente Rejeitado com Sucesso! Uma mensagem foi enviada');</script>";
...
else
{
	...
	$sql = "INSERT into cartao(bandeira, codigo, criacao,id_conta, nome_cliente, numero, validade)values('UVV',
		...
		if (smtpmailer($mail, $from, 'LAPS', 'Cliente Aprovado', $msg)) {
			$volta = $_SERVER['HTTP_REFERER'];
